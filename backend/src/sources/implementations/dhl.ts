...
export class DHLSource extends TrackingSource {
  ...
  async getTracking(params: Record<string, string>): Promise<ShipmentInfo> {
    const response = await fetch(`${this.config.baseUrl}track/shipments?trackingNumber=${params.trackingNumber}`, {
      headers: { 'accept': 'application/json', 'dhl-api-key': this.config.apiKey as string }
    });
